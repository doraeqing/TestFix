...
@interface YTXMFValue()

@property (assign, nonatomic) BOOL structPointNoCopyData;

@end

@implementation YTXMFValue{
    YTXMFTypeSpecifier *_type;
	...
	switch (_type.typeKind) {
		...
		case MF_TYPE_STRUCT:
			if (src.type.typeKind == MF_TYPE_STRUCT) {
				...
				YTXMFStructDeclare *declare = [table getStructDeclareWithName:self.type.structName];
...
- (YTXMFTypeSpecifier *)type{
    return _type;
}


- (void)setType:(YTXMFTypeSpecifier *)type{
    _type = type;
}
