...
static void blockInter(ffi_cif *cif, void *ret, void **args, void *userdata){
	YTXMFBlock *mangoBlock = (__bridge YTXMFBlock *)userdata;
	YTXMFInterpreter *inter = mangoBlock.inter;
	YTXMFScopeChain *scope = [YTXMFScopeChain scopeChainWithNext:mangoBlock.outScope];
	YTXMFFunctionDefinition *func = mangoBlock.func;
	...
	for (NSUInteger i = 1; i < numberOfArguments ; i++) {
		void *arg = args[i];
		YTXMFValue *argValue = [[YTXMFValue alloc] initWithCValuePointer:arg typeEncoding:[sig getArgumentTypeAtIndex:i] bridgeTransfer:NO];
		[argValues addObject:argValue];
		
	}
	__autoreleasing YTXMFValue *retValue = mf_call_mf_function(inter, scope, func, argValues);
	[retValue assignToCValuePointer:ret typeEncoding:[sig methodReturnType]];
}

@implementation YTXMFBlock{
	ffi_cif *_cifPtr;
	ffi_type **_args;
